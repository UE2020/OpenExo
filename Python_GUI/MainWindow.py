...
#     self.stack.setCurrentWidget(self.trial_page)
    ...
    def _on_connect_requested(self, mac: str):
        ...
        try:
            # Clear partial handshake/stream state before notifications for the
            # new link can arrive.
    ...
    def _clear_pending_param_updates(self, reason: str = ""):
        ...
        self.logger.info(
            "Dropped %s pending parameter ack(s)%s",
            pending,
    ...
    def _on_end_trial(self):
        try:
            ...
            # Send one ordered, session-bound shutdown transaction. The device
            # manager disconnects only after all three GATT writes complete.
            try:
                self.qt_dev.endTrialAndDisconnect()
                self.logger.info(
                    "Submitted ordered stop trial, motor off, and reset sequence"
                )
            except Exception as e:
    ...
    def _on_dev_disconnected(self):
        ...
        # Ack waiters belong to the link that just died; a reconnect inside the
        # 5 s timeout window would otherwise let them absorb the next link's acks.
        self._clear_pending_param_updates("device disconnected")
        try:
            self._destroy_controller_db()

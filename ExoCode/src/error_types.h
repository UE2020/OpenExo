    ...
    public:
        ...
        bool check(JointData* _data)
        {
            ...
            if (_data->torque_data_window.size() > _data->torque_data_window_max_size)
            {
                ...
                //Only *successive* out-of-bounds samples count. Without the reset this is a
                //lifetime tally, so two outliers anywhere in a trial latch the error forever.
                if (utils::is_outside_range(_data->torque_reading, bounds.first, bounds.second))
                {
                    _data->torque_failure_count++;
                }
                else
                {
                    _data->torque_failure_count = 0;
                }
            }

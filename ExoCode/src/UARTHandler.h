...
#define MAX_RX_LEN 192      //Bytes. The 71-value configuration frame needs 144 decoded bytes.
...
class UARTHandler
{
    public:
         ...
         * 
         * @return int The number of bytes available in the UART buffer
         */
        int check_for_data();
    ...
    private:
        ...
        uint16_t _get_packed_length(uint8_t msg_id, uint8_t len, uint8_t joint_id, float *data);

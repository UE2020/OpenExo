...
class MotorData 
{
	public:
        ...
        bool do_zero = false;               /**< Flag to zero the position of the motor */
        bool enabled = false;               /**< Motor enable state*/
        bool is_on = false;                 /**< Motor power state */
        bool is_left = false;               /**< Motor side information 1 if on the left, 0 otherwise */
        bool flip_direction = false;        /**< Should the motor direction be flipped, if true torque commands and position/velocity information will be inverted */

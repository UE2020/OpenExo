...
: _Motor(id, exo_data, enable_pin)
{
    ...
    const bool uses_extended_can =
        _motor_data->motor_type ==
        static_cast<uint8_t>(config_defs::motor::AK60v3);
    if (!CAN::getInstance()->register_motor(
            static_cast<uint8_t>(id),
            uses_extended_can))
    {
        logger::println(
            "CAN motor registration failed for ID " +
            String(static_cast<uint8_t>(id)),
            LogLevel::Error);
    }
...
void _CANMotor::read_data()
{
    if (_motor_data->enabled)
    {
        ...
        CAN_message_t msg;
        if (!can->read_for(
                static_cast<uint8_t>(_motor_data->id),
                is_ak60v3,
                msg))
        {
            return;
...
void _CANMotor::send_data(float torque)
{
    ...
    #ifdef SIMPLE_DEBUG
    //Rate-limited so this cannot flood the 500 Hz control loop. Prints per motor.
    {
        const uint32_t now_ms = millis();
        if ((now_ms - _last_diag_ms) >= 500)
        {
            _last_diag_ms = now_ms;
            Serial.print("\nMTR id=");      Serial.print((uint8_t)_motor_data->id);
            Serial.print(" enabled=");       Serial.print(_motor_data->enabled ? 1 : 0);
            Serial.print(" is_on=");         Serial.print(_motor_data->is_on ? 1 : 0);
            Serial.print(" err=");           Serial.print(_error ? 1 : 0);
            Serial.print(" t_ff=");          Serial.print(torque, 3);
            Serial.print(" i_sat=");         Serial.print(i_sat, 3);
            Serial.print(" canOK=");         Serial.print((unsigned long)can->tx_ok_count);
            Serial.print(" canFAIL=");       Serial.print((unsigned long)can->tx_fail_count);
            Serial.print(" rx=");           Serial.print((unsigned long)_rx_count);
            Serial.print(" mi=");           Serial.print(_motor_data->i, 3);
        }
    }
    #endif
...
{
    //Motor power follows the commanded enable state. Without this line is_on is never
    //assigned true anywhere in the firmware, so whether the enable pin is ever driven
    //high depends on the uninitialised value of is_on at construction.
    _motor_data->is_on = _motor_data->enabled;
